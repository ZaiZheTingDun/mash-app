...
- `commands/` 存放 Tauri command 及相关 helper：
  ...
  - `catalog.rs` + `catalog/`：catalog facade；`servants.rs` 负责从者目录解析与 variant 展开，`portraits.rs` 负责从者立绘/头像资源选择与偏好持久化，`craft_essences.rs` 负责 CE catalog 与卡面资源，`metadata.rs`、`mystic_codes.rs`、`skills.rs` 分别负责 metadata localization、御主礼装资料和从者技能资料。
