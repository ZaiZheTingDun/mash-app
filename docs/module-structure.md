...
- `runner/` 按 domain 拆分战斗自动化 state machine：
  ...
  - `support.rs` + `support/`：助战筛选 facade；`class_filter.rs` 负责职阶页签和国服 EXTRA 筛选，`craft_essence.rs` 负责礼装配置、区域计算和校验诊断，`requirements.rs` 负责等级、技能、星图门槛和诊断文本，`scrolling.rs` 负责列表滚动决策、时序和诊断，`runtime.rs` 负责设备交互流程。
