...
| Support skill slots | Project fixed skill-slot offsets from the confirmation-button anchor. | If saturation-based panel kind is ambiguous, try legacy `support_skill_panel_*` template kind detection. | Button-template anchoring and saturation probe are validated across enough normal, append, and Grand rows. |
