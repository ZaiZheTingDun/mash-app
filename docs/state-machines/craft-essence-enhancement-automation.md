...
- 每次自动化运行首次进入手动列表筛选弹窗时，先用 `scroll_bar_enhancement_filter_top.png` 在右侧轨道定位滑块；中心 `y <= 0.16` 视为已在顶部，否则向最顶端拖动一次。检查完成后，本次运行后续再次进入筛选弹窗不再重复处理。选择丸子、底卡和同名副本时只开启 1 星，3/4/5 星始终关闭。
